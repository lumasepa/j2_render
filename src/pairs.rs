...
impl Pairs {
    ...
    pub fn get<T: PartialEq<String>>(&self, key: T) -> Option<String> {
        self.inner.iter().rev().find(|(k, _)| key == *k).map(|(_, v)| v.to_owned())
    }

    pub fn remove_all(&mut self, key: String) {
        let to_remove :Vec<usize>= self.inner.iter()
            .enumerate()
            .filter(|(i, (k, v))| *k == key)
            .map(|(i, (k, v))| i)
            .collect();
    ...
    pub fn try_from_args(mut args: &mut Vec<String>) -> Result<Pairs, WrapError> {
